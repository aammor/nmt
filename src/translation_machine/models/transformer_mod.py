...
class TransformerForSeq2Seq(nn.Module):
    ...
    def forward(self,batch,last_states_encoder=None):
        ...
        if last_states_encoder is None:
            ...
            #apply the encoder
            src_key_padding_mask = [[idx_j>=src_lengths[idx_batch] for idx_j in range(max_src_length)] for idx_batch in range(batch_size)]
            src_key_padding_mask = torch.tensor(src_key_padding_mask,dtype=torch.bool,device="cuda")
            encoder_output = self.transformer_encoder(embeddings_src,src_key_padding_mask=src_key_padding_mask)
        ...
        memory_key_padding_mask = [[idx_j>=src_lengths[idx_instance] for idx_j in range(max_src_length)] for idx_instance in range(batch_size)]
        tgt_key_padding_mask = [[idx_j>=tgt_lengths[idx_batch] for idx_j in range(max_tgt_length)] for idx_batch in range(batch_size)]

        memory_key_padding_mask = torch.tensor(memory_key_padding_mask,dtype=torch.bool,device="cuda")
        tgt_key_padding_mask = torch.tensor(tgt_key_padding_mask,dtype=torch.bool,device="cuda")

        out_decoder = self.transformer_decoder(embeddings_tgt,memory=outputs["last_states_encoder"],tgt_mask=tgt_mask,memory_key_padding_mask=memory_key_padding_mask,tgt_key_padding_mask=tgt_key_padding_mask)
        ...
        #TODO : method to complicated , to be refactored
        ...
        if self.training:
            ...
            with torch.no_grad():
                ...
                outputs["token_targets_insgtance_separated"] = common.divide_into_sublist(outputs["targets"],predictions_lengths)
    ...
    def predict(self,tokens_list_src_int,tokens_list_tgt_int,last_states_encoder=None):
        ...
        outputs = dict()
        outputs["preds_last"] = outputs_model["preds_last"]
