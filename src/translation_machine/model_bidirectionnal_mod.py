...
class SequenceTranslator(nn.Module):
    ...
    def __init__(self,embeddings_src_size,embeddings_tgt_size,hidden_size_encoder,hidden_size_decoder,
                 vocab_src,vocab_tgt,length_src_sentence,length_tgt_sentence,bidirectional_encoder,dropout=0.0):
        super(SequenceTranslator,self).__init__()
        self.src_embedding_layer = nn.Embedding(len(vocab_src),embeddings_src_size)
        self.dst_embedding_layer = nn.Embedding(len(vocab_tgt),embeddings_tgt_size)
        self.length_src_sentence = length_src_sentence
        self.length_tgt_sentence = length_tgt_sentence

        self.encoder = nn.LSTM(input_size=embeddings_src_size,hidden_size=hidden_size_encoder,
                               num_layers=1,batch_first=True,dropout=dropout,
                              bidirectional=bidirectional_encoder)
        self.decoder = nn.LSTM(input_size=embeddings_tgt_size,hidden_size=hidden_size_decoder,
                               num_layers=1,batch_first=True,dropout=dropout)
    ...
    def apply_encoder(self,en_id_tokens_batchs,en_lengths,with_all_outputs=False):
        ...
        """
        en_embeddings = self.src_embedding_layer(en_id_tokens_batchs)
    ...
    def apply_decoder(self,dst_id_tokens_batchs,dst_lengths,last_states_encoder):
        """
            get last state of decoder as padded
        """
        
        dst_embeddings = self.dst_embedding_layer(dst_id_tokens_batchs)
        packed_dst_embeddings = nn.utils.rnn.pack_padded_sequence(dst_embeddings,dst_lengths,batch_first=True,enforce_sorted=False)
        
        if self.encoder.bidirectional :
            tmp = [ torch.moveaxis(el,0,1) for el in last_states_encoder]
            decoder_state_input = [el.reshape(el.shape[0],-1).unsqueeze(0) for el in tmp]
        else:
            decoder_state_input = last_states_encoder
        # import pdb;pdb.set_trace()
        out_decoder,_ = self.decoder(packed_dst_embeddings,decoder_state_input)
    ...
    def forward(self,en_id_tokens_batchs,ge_id_tokens_batchs,en_lengths,ge_lengths,last_states_encoder=None):
        ...
        outputs = {}
        if self.training: # all the token of the output of decoder on the sequence during training (for teacher forcing)
            #import pdb;pdb.set_trace()
            output = self.linear_layer(out_decoder) 
            outputs["decoder"] = output
            outputs["sequence_lengths"] = sequence_lengths
        else: # only the last token of the output of the decoder 
